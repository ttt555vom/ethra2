(function () {
  ...
  function buildReel(clip, index, src) {
    ...
    user.textContent = clip.authorName || "مستخدم";
    var txt = document.createElement("p");
    txt.className = "reel-text";
    txt.textContent = clip.name || "";
  ...
  if (uploadConfirm) {
    uploadConfirm.addEventListener("click", function () {
      if (!App.Auth.currentUser) {
        alert("يجب تسجيل الدخول لتتمكن من نشر مقاطع.");
        App.Auth.login();
        return;
      }
  ...
  function openCommentsModal(clip) {
    if (!commentsModal) return;
    currentClipIdForComments = clip.id;
    
    // عرض اسم الناشر
    var publisherNameEl = document.getElementById("comments-publisher-name");
    if (publisherNameEl) {
      publisherNameEl.textContent = clip.authorName || "مستخدم";
    }

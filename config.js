...
window.EthraApp = (function () {
  ...
  // ==========================================
  // محرك التخزين المحلي (Local IndexedDB Engine)
  // ==========================================
  // ==========================================
  // إعدادات Firebase
  // ==========================================
  // ملاحظة للمستخدم: استبدل الإعدادات التالية بإعدادات مشروعك في Firebase Console
  var firebaseConfig = {
  ...
  var RemoteAPI = {
    ...
    delete: function (id) {
      ...
      return db.collection("clips").doc(id).get().then(function(doc) {
        if (doc.exists) {
          var data = doc.data();
          // 2. حذف الفيديو من Storage
          // نحتاج لاستخراج المسار من URL أو حفظ المسار في Firestore. 
          // للتبسيط، سنحاول الحذف باستخدام URL إذا كان ممكناً أو نعتمد على أننا نعرف النمط
          var fileName = data.id + "_" + data.fileName;
          var storageRef = storage.ref("videos/" + fileName);
          
          return storageRef.delete().catch(function(err) {
            console.warn("فشل حذف الملف من Storage:", err);
          }).then(function() {
            // 3. حذف المستند من Firestore
            return db.collection("clips").doc(id).delete();
          });
        }
      });
    },
